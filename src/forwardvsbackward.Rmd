...
## Methods
We computed the effectiveness of contact tracing as the proportion of generation 3 cases averted. Assuming a negative-binomial branching process with a mean $R$ and overdispersion paramter $k$, The mean total number of generation 3 cases given an index case found by surveillance is $m=R^2(1+R(1+\frac1k))$.
### Forward tracing
...
We computed the effectiveness of tracing as $\frac{\Delta_F}{m}$ and $\frac{\Delta_{F+B}}{m}$.
...
## Main and supplementary figures: the effectiveness of forward and backward contact tracing
...
## Supplementary figures: the absolute number of cases averted by backward tracing
...
### Figure S3. The number of generation 3 cases averted with 80% success rate of tracing and 80% relative reduction in transmission during quarantine.

```{r}
par(mfrow=c(3,3),mar=c(4,3,2.5,1),mgp=c(2,0.6,0),las=0,yaxs="i")

tertiary<-plotbyRk(q=0.8,c=0.8,b=0.8,d=0.2, panel=c("(A)","(B)","(C)"))
tertiary<-plotbyRk(q=0.8,c=0.8,b=0.5,d=0.2, panel=c("(D)","(E)","(F)"))
tertiary<-plotbyRk(q=0.8,c=0.8,b=0.8,d=0.1, panel=c("(G)","(H)","(I)"))
```

### Figure S4. The number of generation 3 cases averted with 60% success rate of tracing and 60% relative reduction in transmission during quarantine.

```{r}
par(mfrow=c(3,3),mar=c(4,3,2.5,1),mgp=c(2,0.6,0),las=0,yaxs="i")

tertiary<-plotbyRk(q=0.6,c=0.6,b=0.8,d=0.2, panel=c("(A)","(B)","(C)"))
tertiary<-plotbyRk(q=0.6,c=0.6,b=0.5,d=0.2, panel=c("(D)","(E)","(F)"))
tertiary<-plotbyRk(q=0.6,c=0.6,b=0.8,d=0.1, panel=c("(G)","(H)","(I)"))
```
